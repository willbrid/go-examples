...
La bibliothèque standard Go prend en charge l'écriture de tests unitaires via le package `testing`. Les tests unitaires sont
exprimés sous forme de fonctions dont le nom commence par `Test`, suivi d'un terme commençant par une lettre majuscule, comme TestSum.
...
Les fonctions de test unitaire reçoivent un pointeur vers une structure T (`testing.T`), qui définit les méthodes de gestion des tests
et de rapport des résultats des tests. Les tests Go ne reposent pas sur des assertions et sont écrits à l'aide d'instructions de
code standard. Tout ce dont les outils de tests se soucient, c'est de savoir si le test échoue, ce qui est signalé à l'aide des méthodes :
...
La commande `go test` ne rapporte aucun détail par défaut, mais plus d'informations peuvent être générées en exécutant la commande :
`go test -v`

La commande `go test`peut être utilisée pour exécuter des tests sélectionnés par leur nom : `go test -v -run "TestSum"`
Les tests sont sélectionnés avec une expression régulière. Le seul test dont le nom correspond à l'expression est <TestSum>.

La structure T (`testing.T`) fournit également un ensemble de méthodes pour gérer l'exécution des tests :
- Run(name, func) : l'appel de cette méthode exécute la fonction spécifiée en tant que `sous-test`. La méthode se bloque pendant que
					le test est exécuté dans sa propre goroutine et renvoie un booléen qui indique si le test a réussi.
					La méthode `Run` est utilisée pour exécuter un sous-test, ce qui est un moyen pratique d'exécuter une
					série de tests liés à partir d'une seule fonction.
